...
final class ReportGenUtil {
	...
	static List<ServiceCost> splitServiceCostsByReport(DateRange reportDateRange, List<ServiceCost> serviceCosts) {
		...
		for (ServiceCost cost : serviceCosts) {
			cost.dateRange().intersect(reportDateRange)
	...
	static <T> void validateDateRangeCoverage(DateRange reportDateRange, List<T> items,
			Function<T, DateRange> dateRangeExtractor, String itemName) {

		List<T> sortedItems = items.stream()
				.sorted(Comparator.comparing(item -> dateRangeExtractor.apply(item).startDate())).toList();

		LocalDate sortedFirstStart = dateRangeExtractor.apply(sortedItems.getFirst()).startDate();
		LocalDate sortedLastEnd = dateRangeExtractor.apply(sortedItems.getLast()).endDateExclusive();
		...
		for (int i = 0; i < sortedItems.size() - 1; i++) {
			...
			if (!currentEnd.equals(nextStart)) {
				throw new IllegalArgumentException(
						String.format("%s do not connect seamlessly or they overlap: %s and %s", itemName,
								sortedItems.get(i), sortedItems.get(i + 1)));
			}
		}
	}
	...
	static List<FeeCalculationResult> calculateFees(DateRange reportDateRange, List<ServiceCost> serviceCosts) {
		return splitServiceCostsByReport(reportDateRange, serviceCosts).stream().map(ReportGenUtil::calculateFee)
				.toList();
	}

	private static FeeCalculationResult calculateFee(ServiceCost serviceCost) {
		DateRange dateRange = serviceCost.dateRange();
		BigDecimal monthlyCost = serviceCost.annualCost().divide(MONTHS_IN_YEAR, CALCULATION_SCALE,
	...
	public record FeeCalculationResult(DateRange dateRange, BigDecimal monthCount, BigDecimal monthlyCost,
