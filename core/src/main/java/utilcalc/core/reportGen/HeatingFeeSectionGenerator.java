...
final class HeatingFeeSectionGenerator {
    ...
    static HeatingFeeSection generateHeatingFeeSection(DateRange reportDateRange, HeatingFeeInputs heatingFeeInputs) {
        ...
        List<HeatingFee> heatingFees = monthlyServiceCost.stream()
                .map(HeatingFeeSectionGenerator::calculateHeatingFee)
                .toList();

        BigDecimal totalAmount =
                calculateAmount(heatingFees, HeatingFee::feeAmount).setScale(DISPLAY_DECIMAL_PLACES, ROUNDING_MODE);
    ...
    private static List<HeatingFee> toRoundedFees(List<HeatingFee> heatingFees) {
        return heatingFees.stream()
                .map(HeatingFeeSectionGenerator::toRoundedFee)
                .toList();
    }
