...
class DateRangeParser {
	...
	private static DateRange parseInterval(String from, String to) {
		if (YEAR_MONTH_DAY_PATTERN.matcher(from).matches() && YEAR_MONTH_DAY_PATTERN.matcher(to).matches()) {
			LocalDate start = LocalDate.parse(from);
			LocalDate end = LocalDate.parse(to).plusDays(1);
			return new DateRange(start, end);
		} else if (YEAR_MONTH_PATTERN.matcher(from).matches() && YEAR_MONTH_PATTERN.matcher(to).matches()) {
