		...
		while (reader.hasMore()) {
			...
			switch (header.name().toLowerCase()) {
				...
				case DepositsSectionParser.SECTION_NAME ->
					sections.add(DepositsSectionParser.parse(header, reader.readAllUntilNextGroup()));
				case HeatingSectionParser.SECTION_NAME ->
					sections.add(HeatingSectionParser.parse(header, reader.readAllUntilNextGroup()));
				case OtherFeesSectionParser.SECTION_NAME ->
					sections.add(OtherFeesSectionParser.parse(header, reader.readAllUntilNextGroup()));
				case ColdWaterSectionParser.SECTION_NAME ->
					sections.add(ColdWaterSectionParser.parse(header, reader.readAllUntilNextGroup()));
				case HotWaterSectionParser.SECTION_NAME ->
					sections.add(HotWaterSectionParser.parse(header, reader.readAllUntilNextGroup()));
