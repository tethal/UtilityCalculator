...
class RealDataIntegrationTest {
	...
	void testGoldenFiles(String name) {
		TestHelpers.goldenTest("real/" + name,
				src -> new Dumper().dumpReport(ReportGen.generateReport(Parser.parse(src))).toString());
	}
	...
	private static final class Dumper {
		...
		Dumper dumpReport(Report report) {
			...
			dumpStringList("placeDate", List.of(report.reportPlace(), report.reportDate().toString()));
		...
		private void dumpDepositSection(DepositSection section) {
			dumpTable(section.deposits(), "  %: % * % = %\n", Deposit::description, Deposit::count, Deposit::unitAmount,
					Deposit::amount);
		}
		...
		private void dumpHotWaterSection(HotWaterSection section) {
			...
			dumpTable(section.heatingBasicParts(), "    %: % * % = %\n", WaterHeatingBasicPart::dateRange,
					WaterHeatingBasicPart::numberOfMonths, WaterHeatingBasicPart::monthlyCost,
					WaterHeatingBasicPart::totalAmount);

			append("  heatingConsumableParts:\n");
			dumpTable(section.heatingConsumableParts(), "    %: % * % = %\n", WaterHeatingConsumablePart::dateRange,
					WaterHeatingConsumablePart::unitAmount, WaterHeatingConsumablePart::unitCost,
					WaterHeatingConsumablePart::totalCost);
		}

		private void dumpHeatingFeeSection(HeatingFeeSection section) {
			dumpTable(section.fees(), "  %: % * % * % = %\n", HeatingFee::yearMonth, HeatingFee::monthCount,
					HeatingFee::annualCost, HeatingFee::coefficient, HeatingFee::feeAmount);
		}

		private void dumpOtherFeeSection(OtherFeeSection section) {
			dumpTable(section.fees(), "  %: % * % = %\n", OtherFee::dateRange, OtherFee::monthCount,
					OtherFee::monthlyCost, OtherFee::feeAmount);
		}

		private void dumpWaterReadings(List<WaterReading> readings) {
			append("  readings:\n");
			dumpTable(readings, "    [%] %: %...% = %\n", WaterReading::meterId, WaterReading::dateRange,
					WaterReading::startState, WaterReading::endState, WaterReading::consumption);
		}

		private void dumpWaterFees(List<WaterFee> waterFees) {
			append("  priceList:\n");
			dumpTable(waterFees, "    %: % * % = %\n", WaterFee::dateRange, WaterFee::quantity, WaterFee::unitAmount,
		...
		private void dumpStringList(String label, List<String> stringList) {
			append("  ").append(label).append(": ").append(String.join(", ", stringList)).append("\n");
		}
