...
public final class PdfGenerator {
	...
	public static String buildHtml(Report report) {
		...
		html.lineBreak().append(report.reportPlace() + ", " + formatter.formatDate(report.reportDate())).lineBreak();
	...
	private static void generateSummary(HtmlBuilder html, Report report) {
		...
		report.sections().forEach(s -> html.beginTr().tdText(s.name()).tdMoney(s.totalAmount()).endTr());
		html.totalRow(1, report.total().signum() == -1 ? "Přeplatek" : "Nedoplatek", report.total().abs());
		html.endTable();
		html.endSection();
	...
	private static void appendDepositsTable(HtmlBuilder html, DepositSection depositSection) {
		boolean unitCount = depositSection.deposits().stream().anyMatch(d -> d.count().compareTo(BigDecimal.ONE) != 0);
	...
	private static void appendOtherFeeTable(HtmlBuilder html, OtherFeeSection otherFeeSection) {
		...
		for (OtherFee fee : otherFeeSection.fees()) {
			html.beginTr().tdDateRange(fee.dateRange()).tdMonths(fee.monthCount()).tdCzkPerMonth(fee.monthlyCost())
					.tdMoney(fee.feeAmount()).endTr();
		}
	...
	private static void appendHeatingFeeTable(HtmlBuilder html, HeatingFeeSection heatingFeeSection) {
		...
		for (HeatingFee fee : heatingFeeSection.fees()) {
			html.beginTr().tdYearMonth(fee.yearMonth()).tdNumberWithPercent(fee.monthCount(), fee.coefficient())
					.tdCzkPerYear(fee.annualCost()).tdMoney(fee.feeAmount()).endTr();
		}
	...
	private static void appendColdWaterTable(HtmlBuilder html, ColdWaterSection section) {
		...
		for (WaterFee fee : section.priceList()) {
			html.beginTr().tdDateRange(fee.dateRange()).tdCubicMeter(fee.quantity())
					.tdCzkPerCubicMeter(fee.unitAmount()).tdMoney(fee.periodAmount()).endTr();
		}
	...
	private static void appendHotWaterTable(HtmlBuilder html, HotWaterSection section) {
		...
		for (WaterFee fee : section.priceList()) {
			html.beginTr().tdDateRangeWithTitle("Studená voda ", fee.dateRange()).tdCubicMeter(fee.quantity())
					.tdCzkPerCubicMeter(fee.unitAmount()).tdMoney(fee.periodAmount()).endTr();
		}

		for (WaterHeatingBasicPart part : section.heatingBasicParts()) {
			html.beginTr().tdDateRangeWithTitle("Ohřev základní složka ", part.dateRange())
					.tdMonths(part.numberOfMonths()).tdCzkPerMonth(part.monthlyCost()).tdMoney(part.totalAmount())
					.endTr();
		}

		for (WaterHeatingConsumablePart part : section.heatingConsumableParts()) {
			html.beginTr().tdDateRangeWithTitle("Ohřev spotřební složka ", part.dateRange())
					.tdCubicMeter(part.unitAmount()).tdCzkPerCubicMeter(part.unitCost()).tdMoney(part.totalCost())
					.endTr();
		}
