...
final class WaterSectionUtil {
	...
	public static List<WaterReading> generateWaterReadings(DateRange reportDateRange,
		...
		return meterReadings.stream().collect(Collectors.groupingBy(MeterReading::meterId)).values().stream()
				.flatMap(group -> createColdWaterReadings(reportDateRange, group).stream()).toList();
	}
	...
	private static List<WaterReading> createColdWaterReadings(DateRange reportDateRange, List<MeterReading> readings) {
		...
		List<MeterReading> sortedMeterReadings = readings.stream()
				.sorted(Comparator.comparing(MeterReading::readingDate)).collect(Collectors.toList());

		LinkedList<WaterReading> waterReadings = new LinkedList<>(
	...
	private static void addMissingStartWaterReading(DateRange reportDateRange, List<WaterReading> readings,
			boolean isInnerReading) {
		WaterReading first = readings.getFirst();
		WaterReading second = readings.get(1);
		LocalDate endDateExclusive = isInnerReading ? first.dateRange().startDate() : second.dateRange().startDate();
		...
		WaterReading newWaterReading = new WaterReading(newDateRange, first.meterId(), newStartState, startState,
				newConsumption);
	...
	private static void addMissingEndWaterReading(DateRange reportDateRange, List<WaterReading> readings,
			boolean isInnerReading) {
		...
		WaterReading newWaterReading = new WaterReading(newDateRange, last.meterId(), endState, newEndState,
				newConsumption);
		if (!isInnerReading)
			readings.removeLast();
	...
	public static List<WaterFee> generatePriceList(DateRange reportDateRange, List<WaterTariff> waterTariffs,
			List<WaterReading> waterReadings) {

		validateDateRangeCoverage(reportDateRange, waterTariffs, WaterTariff::dateRange, "WaterTariff");

		return waterTariffs.stream().sorted(Comparator.comparing(WaterTariff::dateRange))
				.flatMap(waterTariff -> createColdWaterFees(waterTariff, waterReadings).stream()).toList();
	}
	...
	private static WaterFee createColdWaterFee(BigDecimal pricePerCubicMeter, List<WaterReading> waterReadings,
			DateRange interval) {
	...
	private static List<DateRange> extractValidIntervals(WaterTariff waterTariff, List<WaterReading> meterReadings) {
		...
		Set<LocalDate> datePoints = meterReadings.stream().flatMap(reading -> reading.dateRange().stream())
		...
		return IntStream.range(0, sortedDates.size() - 1)
				...
				.map(interval -> interval.intersect(waterTariff.dateRange())).flatMap(Optional::stream).toList();
	}
	...
	private static BigDecimal calculateQuantity(List<WaterReading> detailWaterReadings, DateRange interval) {
		return detailWaterReadings.stream()
				.map(reading -> reading.dateRange().intersect(interval)
						.map(overlap -> calculateConsumption(overlap, reading)).orElse(BigDecimal.ZERO))
