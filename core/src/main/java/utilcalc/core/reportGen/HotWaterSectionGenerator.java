...
final class HotWaterSectionGenerator {
	...
	static HotWaterSection generateHotWaterSection(DateRange reportDateRange,
			HotWaterSectionInputs hotWaterSectionInputs) {
		...
		List<WaterHeatingBasicPart> heatingBasicParts = calculateFees(reportDateRange,
				hotWaterSectionInputs.heatingBasicCosts()).stream()
				.map(HotWaterSectionGenerator::mapFeeResultToHeatingBasicPart).toList();

		List<WaterHeatingConsumablePart> heatingConsumableParts = generatePriceList(reportDateRange,
				hotWaterSectionInputs.heatingConsumableTariffs(), readings).stream()
				.map(HotWaterSectionGenerator::mapWaterFeeToWaterHeatingConsumablePart).toList();
		...
		BigDecimal consumablePartsAmount = calculateAmount(heatingConsumableParts,
		...
		return new HotWaterSection(hotWaterSectionInputs.name(), totalAmount, readings, priceList, heatingBasicParts,
	...
	private static WaterHeatingBasicPart mapFeeResultToHeatingBasicPart(FeeCalculationResult feeCalculationResult) {
		return new WaterHeatingBasicPart(feeCalculationResult.dateRange(), feeCalculationResult.monthCount(),
				feeCalculationResult.monthlyCost(), feeCalculationResult.feeAmount());
	}

	private static WaterHeatingConsumablePart mapWaterFeeToWaterHeatingConsumablePart(WaterFee waterFee) {
		return new WaterHeatingConsumablePart(waterFee.dateRange(), waterFee.quantity(), waterFee.unitAmount(),
